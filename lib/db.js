...
Store.prototype.identify = function (object) {
  ...
  function set(object) {
    if(object._id) {
      ...
      if (typeof u !== 'object') {
        if (typeof u === 'string' && u.length === 24 && uuid.isObjectID(u)) {
          object._id = uuid.toObjectID(u);
          delete object.id;
        }
      } else {
        if (Array.isArray(u)) {
          u = u.map(function(v) {
            return (typeof v === 'string' && v.length === 24 && uuid.isObjectID(v)) ? uuid.toObjectID(v) : v;
          });
        } else {
          Object.keys(u).forEach(function(k){
            if (Array.isArray(u[k])) {
              u[k] = u[k].map(function(v) {
                return (typeof v === 'string' && v.length === 24 && uuid.isObjectID(v)) ? uuid.toObjectID(v) : v;
              });
            } else {
              u[k] = (typeof u[k] === 'string' && u[k].length === 24 && uuid.isObjectID(u[k])) ? uuid.toObjectID(u[k]) : u[k];
            }
          });
        }
        object._id = u;
