...
Store.prototype.scrubQuery = function (query) {
  ...
  if(query.id) {
    query._id = uuid.toObjectID(query.id);
    delete query.id;
  }
  ...
  try {
    scrub(query, function (obj, key, parent, type) {
      ...
      if(key === 'id' && parent.id && typeof(parent.id) === 'string' && parent.id.length === 24 && uuid.isObjectID(parent.id)) {
        parent._id = uuid.toObjectID(parent.id);
        delete parent.id;
      }
    });
  } catch(ex) {
