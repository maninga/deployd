/*globals SHIFT_KEYS:false, TEXT_KEYS:false, NUMBER_KEYS:false,
          alert:false, date:false, moment:false */
(function() {
  ...
  vm.edit.getEditorMode = function(){
    // var mode = isJson() ? 'json' : 'text';
    var type = vm.selectedProp().type;
    return (type === 'object' || type === 'array') ? 'json' : (type === 'function' ? 'javascript' : 'text');
  };

  vm.edit.isEditableInline = function() {
    if (vm.edit.getEditorMode() !== 'text') return false;
  ...
  vm.edit.isEditableModal = function() {
    var type = vm.selectedProp().type;
    return (vm.edit.getEditorMode() !== 'text' || type === 'string');
  };
    ...
    setTimeout(function() {
      ...
      if (type === 'function') {
        ...
        if (newVal.trim() === '') {
          ...
          try {
            var tempVal = newVal;
            newVal = moment(tempVal);
            if (!newVal.isValid()) {
              newVal = moment(date(tempVal));
            }
          } catch (ex) {}
  ...
  vm.edit.start = function(newVal, options) {
    ...
    setTimeout(function() {
      ...
      if (editType === 'date' || editType === 'function') {
        val = vm.edit.editProp();
        ...
        val = vm.edit.editProp();
        if (editType === 'date' && !val) val = "";
        if (editType === 'string' && !val) val = "";
        if (editType === 'number' && !val) val = 0;
        vm.edit.editValue(val);
        vm.postbox.notifySubscribers(true, 'selectEditor');
      }
  ...
  function createRow(data) {
    ...
    rowVm._textFor = function(prop) {
      ...
      if (prop.type === 'boolean') {
        ...
        return '...';
      } else if (prop.type === 'date' && typeof val === 'object') {
