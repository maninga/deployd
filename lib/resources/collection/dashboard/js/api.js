  ...
  $('#api').show().html(apiTemplate({
    ...
    , getObj: function() {
        var props = res.properties
          ...
          , obj = {}
          , key
          , vals = {string: 'foobar', number: 123, boolean: true, date: new Date(), function: 'function(){\n}'}
        ;

        while(key = keys.shift()) {
          if(props[key].type !== 'date') {
            obj[key] = vals[props[key].type];
          }
          else {
            obj[key] = vals[props[key].type].toISOString();
          }
        }
